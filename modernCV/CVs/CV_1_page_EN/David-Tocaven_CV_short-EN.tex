...
\item \textbf{Modelling: }%Modélisation
 {\small frequency, state space, linear and non linear, linear multiple input-output, of observer, uncertain, of late system.}%fréquentielle, d'espace d'état, linéaire et non linéaire, linéaire multivariable, d'observateurs, incertaine, de système à retard.
% 
\item \textbf{Analysis: }%Analyse:
{\small frequency, Lyapunov theory, performance, linear and non-linear system, uncertain system, robustness, stability of late system.}%fréquentielle, Théorie de Lyapunov, de performance, de système linéaire et non linéaire, de système incertain, de robustesse, de stabilité sur systèmes à retard.
\item \textbf{Control: }%Synthèse de commande:
{\small PID, multiple input-output, robust, Full state feedback, late system}%PID, multivariable, robuste, par retour d'état, sur système a retard.
\end{itemize}}}{Automatic control -- Discrete events systems}%Automatique -- Systèmes à événements discret :
{{\begin{itemize}[label=\myitem]%
		...
		\item {\small DEVS models,}%Modèles DEVS
		\item {\small control}%Synthèse de commande,
		\item {\small Analysis, simulation, implementation,}%Analyse, simulation, implémentation,
		\item {\small Diagnosers and controllability,}%Diagnostiqueur et Contrôlabilité,
		\item {\small Language,}%Langage,
		\item {\small Supervised control,}%Commande supervisée,
%		\item {\small Mise en \oe uvre logicielle/matérielle}%
		\end{itemize}}}{Implementation :}%Mise en \oe uvre :
{{%
		\begin{itemize}[label=\myitem]%
...
{{%
	\begin{itemize}[label=\myitem]%
	\item \textbf{Langue: } {\small French (mother tongue),  English}%{Langue: } {\small Anglais B2}%
